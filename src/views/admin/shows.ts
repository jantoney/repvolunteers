...
export function renderShowsTemplate(data: ShowsPageData): string {
  ...
  return `
    ...
    <body>
      ...
      <!-- Main Content -->
      <div class="main-content productions-page">
    ...
    shows
      .map((show) => {
        ...
        return `                    <tr${rowClass}>                      <td data-label="Production">
                        <strong>${show.name}</strong>
                        ${
          show.show_date_count > 0
            ? `<br><button class="expand-btn btn btn-sm btn-secondary" onclick="toggleDates(${show.id})">View ${show.show_date_count} performance(s)</button>`
            : ""
        }
                      </td>
                      <td data-label="Performances">${show.show_date_count} performance${
