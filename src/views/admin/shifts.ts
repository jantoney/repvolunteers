  ...
  try {
    ...
    let shiftsQuery = `
            SELECT s.id, s.show_date_id, sh.id as show_id, sh.name as show_name, 
              ...
              COUNT(DISTINCT vs.participant_id) as volunteer_count,
              COALESCE(
      ...
      JOIN shows sh ON sh.id = sd.show_id
            ...
            LEFT JOIN participants p ON p.id = vs.participant_id
    `;
    ...
    for (const shift of shiftsResult.rows) {
      ...
      const volunteerNames = Array.isArray(shift.volunteer_names)
