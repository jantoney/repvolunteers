...
export async function showShiftsPage(ctx: RouterContext<string>) {
  ...
  try {
    ...
    const includePastParam = ctx.request.url.searchParams.get("includePast");
    const includePast =
    ...
    if (!includePast && !selectedDate) {
      whereConditions.push(
        `DATE(sd.start_time AT TIME ZONE 'Australia/Adelaide') >= DATE(NOW() AT TIME ZONE 'Australia/Adelaide')`
      );
    }
