...
export async function generateUnfilledShiftsPDF(): Promise<Uint8Array> {
  try {
    ...
    if (data.totalShifts === 0) {
      ...
      doc.setFont("helvetica", "normal");
      doc.text(
        "Excellent! All shifts currently have participants assigned.",
        margin,
        yPos,
      );
      yPos += 8;
      ...
      const alertLine2 = `${data.totalShifts} unfilled shift${
        data.totalShifts !== 1 ? "s" : ""
      } need${data.totalShifts === 1 ? "s" : ""} participants assigned.`;
      ...
      for (const [_key, shiftsGroup] of grouped.entries()) {
        ...
        for (let i = 0; i < shiftsGroup.length; i++) {
          ...
          if (i === 0) {
            const formattedDate = new Date(firstShift.date).toLocaleDateString(
              "en-AU",
              {
                day: "2-digit",
                month: "2-digit",
          ...
          if (i === 0) {
            doc.text(
              `${firstShift.show_start} - ${firstShift.show_end}`,
              xPos,
              yPos,
            );
          }
...
export async function generateOutstandingShiftsPDF(
  ...
  try {
    ...
    if (data.totalShifts === 0) {
      ...
      doc.setFont("helvetica", "normal");
      doc.text(
        "Excellent! All upcoming shifts currently have participants assigned.",
        margin,
        yPos,
      );
      yPos += 8;
      ...
      doc.setFont("helvetica", "bold");
      doc.text(
        `We need your help! ${data.totalShifts} shifts need volunteers.`,
        margin + 3,
        yPos + 5,
      );
      doc.setFont("helvetica", "normal");
      ...
      doc.setFont("helvetica", "bold");
      doc.text(
        "To confirm last minute shifts or last minute sickness:",
        margin + 3,
        yPos + 6,
      );
      doc.setFontSize(14);
...
export async function generateOutstandingShiftsPDFForVolunteer(
  ...
  try {
    ...
    if (data.totalShifts === 0) {
      ...
      doc.setFont("helvetica", "normal");
      doc.text(
        "Either all shifts are filled, or the remaining unfilled shifts",
        margin,
        yPos,
      );
      yPos += 6;
      doc.text(
        "conflict with your current schedule. Thank you for volunteering!",
        margin,
        yPos,
      );
    } else {
      ...
      doc.setFont("helvetica", "bold");
      doc.text(
        `${data.totalShifts} shifts available that don't conflict with your schedule!`,
        margin + 3,
        yPos + 5,
      );
      doc.setFont("helvetica", "normal");
...
async function getOutstandingShiftsData(
  limit: number = 10,
